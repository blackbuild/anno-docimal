...
3. Run the local product audit and signing configuration inspection. It checks the complete unsigned product and that signing/staging/Plugin Portal tasks are configured, but it neither reads credentials nor creates signatures.
...
4. From the protected RC environment, sign and publish all six Maven artifacts as one atomic-as-possible operation, then publish both Plugin Portal markers for that exact version. Do not make a tag, GitHub prerelease, public Pages snapshot, alias, or status claim during a partially completed registry operation.
