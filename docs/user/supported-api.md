...
The supported Java API is non-null by default under JSpecify 1.0.0. Each currently implemented supported top-level type
is `@NullMarked`: `AnnoDoc`, `InlineJavadocs`, `AstDocumentation`, `Documentation`, `SourceProjector`,
`ProjectionPolicy`, `DeclarationVisibility`, `SourceProjectionException`, and `SourceProjectionTask`. Supported nested
builders and value types inherit the null-marked scope of `Documentation` or `ProjectionPolicy`.

Type-level marking is intentional. These packages also contain implementation-only classes, so package- or module-level
marking would incorrectly promote their unspecified nullness into supported API. Public raw nullable values are limited
to contracts such as the comparison argument of value-based `equals(Object)` and are explicitly `@Nullable`. Absent
documentation scalars and declaration identifiers remain non-null `Optional` results; collection members, map entries,
paths, documentation text, and builder/facade arguments remain non-null.

JSpecify metadata is a compile-time/tooling contract, not a replacement for runtime validation. Java and Groovy
consumers continue to receive the same null rejection, explicit empty/clear operations, and template validation. The
published artifacts expose the JSpecify dependency; the generator keeps it external to its shadow JAR to avoid a split
package on the module path. JetBrains `@Language` annotations in tests remain unrelated language-injection metadata.

ADR 0059 records the decision and its publication constraints.
...
applies Gradle's Groovy plugin and the base layer. Plugin implementation classes remain implementation-only.
...
The scoped `1.0-authoring-api-baseline.txt`, `1.0-projection-api-baseline.txt`, and `1.0-gradle-api-baseline.txt` checks
record their selected signatures plus JSpecify `@NullMarked` declarations and public `@Nullable` parameter contracts.
They remain evidence rather than the
complete mechanism: release-baseline work must additionally capture every artifact's constructors, fields, generic
signatures, type relationships, annotation contracts/defaults, nested-type membership, and the other rules above.
