...
belong to the caller; they are not implicit singleton behavior of `SourceProjector`. Issue #35 owns the future reusable
Gradle task that will take on a managed collection output.
...
The directory operation writes one UTF-8/LF package-relative source file and never cleans sibling output. Existing code
that depended on collection scanning or stale-output cleanup must keep that responsibility in its caller until issue #35
delivers its supported Gradle task contract.

## Gradle migration

The Gradle clean cut is not deliverable yet. Current builds may use
`com.blackbuild.annodocimal.base-plugin`, which registers `createClassStubs` and makes `javadoc` consume its output in a
Java-model project, or `com.blackbuild.annodocimal.plugin`, which also applies the current compiler-option convention.
Neither the current task type nor either implementation class is a supported 1.0 Java API.

Do not migrate to `SourceProjectionTask` or `com.blackbuild.annodocimal.groovy-plugin` yet: both are future names owned
by issue #35. That issue will define its properties, independent registration, managed output cleanup, cache behavior,
and the replacement plugin ID. For an IDE-only mirror today, keep class-file selection, output cleanup, and IDE model
wiring in the consuming build; do not compile, package, or publish the mirror as a second API.
...
- `AnnoDocGenerator` to `SourceProjector`; keep `CreateKlumDslSourceMirrors` consumer-owned until #35 delivers its
  task contract;
- application by `AnnoDocimalPlugin.class` to the current plugin ID until #35 delivers the replacement ID; and
