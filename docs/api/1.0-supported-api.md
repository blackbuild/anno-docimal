...
supported. Issues #38 and #39 have delivered their scoped allowlists; issue #35 owns the still-undelivered Gradle task
allowlist. Release compatibility work generates the complete mechanical snapshots from those classifications.
...
`ProjectionPolicy`, `DeclarationVisibility`, and `SourceProjectionException`. Supported nested builders and value types
inherit the null-marked scope of `Documentation` or `ProjectionPolicy`. The supported `SourceProjectionTask` must use the
same contract when issue #35 introduces it.
...
No 1.0 Gradle Java API allowlist is implemented yet. The current published artifact exposes
`CreateClassStubs`, `AnnoDocimalBasePlugin`, and `AnnoDocimalPlugin`, but all three are implementation-only. Its
currently published plugin IDs are `com.blackbuild.annodocimal.base-plugin` and
`com.blackbuild.annodocimal.plugin`; the latter remains the current opinionated plugin ID, not a stable 1.0 contract.

Issue #35 owns the future supported `SourceProjectionTask`, the replacement
`com.blackbuild.annodocimal.groovy-plugin` ID, their exact property allowlist, and the reusable task behavior described
by ADR 0055. Documentation must not present those future names as available before that issue delivers them.
...
The scoped `1.0-authoring-api-baseline.txt` and `1.0-projection-api-baseline.txt` checks record their selected signatures
plus JSpecify `@NullMarked` declarations and public `@Nullable` parameter contracts. They remain evidence rather than the
